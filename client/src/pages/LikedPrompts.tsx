...
export default function LikedPrompts() {
  const [searchQuery, setSearchQuery] = useState("");

  // Fetch liked prompts
  const { data: likedPrompts = [], isLoading } = useQuery<Prompt[]>({
    queryKey: ["/api/user/likes"],
  });

  // Filter prompts based on search query
  const filteredPrompts = likedPrompts.filter(prompt => {
    ...
    <div className="container mx-auto px-2 py-2 sm:px-3 sm:py-3 md:px-6 md:py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div className="mb-4 md:mb-0">
            ...
            <p className="text-sm md:text-base text-muted-foreground">
              All the prompts you've liked from the community
            </p>
          </div>
          ...
          <div className="flex items-center space-x-2">
            <input
              ...
              data-testid="input-search-liked"
            />
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="text-center py-8">
          <p className="text-muted-foreground">Loading your liked prompts...</p>
        </div>
        ...
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground">
              No liked prompts match your search.
            </p>
          </CardContent>
        </Card>
      ) : (
