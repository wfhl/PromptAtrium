...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/keywords', async (req, res) => {
    try {
      ...
      const keywords = await storage.getKeywords(options);
      res.json(keywords);
    } catch (error) {
  ...
  app.get('/api/keywords/search', async (req, res) => {
    try {
      ...
      const keywords = await storage.searchKeywords(q as string, parseInt(limit as string));
      res.json(keywords);
    } catch (error) {
  ...
  app.get('/api/keywords/:id', async (req, res) => {
    try {
      ...
      const related = await storage.getRelatedKeywords(req.params.id, 10);
      
      res.json({ ...keyword, relatedKeywords: related });
    } catch (error) {
  ...
  app.post('/api/keywords', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const keywordData = insertKeywordSchema.parse({
        ...req.body,
        userId: req.body.type === 'user' ? userId : req.body.userId,
      });
  ...
  app.put('/api/keywords/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      ...
      if (keyword.userId !== userId && keyword.type !== 'system') {
        const user = await storage.getUser(userId);
      ...
      const updated = await storage.updateKeyword(req.params.id, keywordData);
      res.json(updated);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid keyword data", errors: error.errors });
  ...
  app.post('/api/keywords/:id/usage', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const { promptId, templateId, context } = req.body;
      
      const usageData = insertKeywordUsageHistorySchema.parse({
        userId,
      ...
      const usage = await storage.trackKeywordUsage(usageData);
      ...
      res.status(500).json({ message: "Failed to track keyword usage" });
    }
  });

  // Keyword Categories API
  app.get('/api/keyword-categories', async (req, res) => {
    try {
      const { parentId, userId, communityId, type, isActive } = req.query;
      ...
      const categories = await storage.getKeywordCategories(options);
      res.json(categories);
    } catch (error) {
      console.error("Error fetching keyword categories:", error);
      res.status(500).json({ message: "Failed to fetch keyword categories" });
    }
  });

  app.get('/api/keyword-categories/tree', async (req, res) => {
    try {
      const { categoryId } = req.query;
      const hierarchy = await storage.getCategoryHierarchy(categoryId as string);
      res.json(hierarchy);
    } catch (error) {
  ...
  app.get('/api/keyword-categories/:id', async (req, res) => {
    try {
      const category = await storage.getKeywordCategory(req.params.id);
      if (!category) {
        return res.status(404).json({ message: "Category not found" });
      }
      
      // Get children categories
      const children = await storage.getKeywordCategories({ parentId: req.params.id });
      
      res.json({ ...category, children });
    } catch (error) {
      console.error("Error fetching keyword category:", error);
      res.status(500).json({ message: "Failed to fetch keyword category" });
    }
  });

  app.post('/api/keyword-categories', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const categoryData = insertKeywordCategorySchema.parse({
        ...req.body,
        userId: req.body.type === 'user' ? userId : req.body.userId,
      });
      
      const category = await storage.createKeywordCategory(categoryData);
      res.status(201).json(category);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid category data", errors: error.errors });
      ...
      res.status(500).json({ message: "Failed to create keyword category" });
    }
  });

  app.put('/api/keyword-categories/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      ...
      if (category.userId !== userId && category.type !== 'system') {
        const user = await storage.getUser(userId);
        if (!user || user.role !== 'super_admin') {
      ...
      const categoryData = insertKeywordCategorySchema.partial().parse(req.body);
      const updated = await storage.updateKeywordCategory(req.params.id, categoryData);
      res.json(updated);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid category data", errors: error.errors });
      ...
      res.status(500).json({ message: "Failed to update keyword category" });
    }
  });

  app.delete('/api/keyword-categories/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      ...
      await storage.deleteKeywordCategory(req.params.id);
      res.json({ message: "Category deleted successfully" });
    } catch (error) {
      console.error("Error deleting keyword category:", error);
      res.status(500).json({ message: "Failed to delete keyword category" });
    }
  });
  ...
  app.get('/api/prompt-templates', async (req, res) => {
    try {
      ...
      const templates = await storage.getPromptTemplates(options);
      res.json(templates);
    } catch (error) {
  ...
  app.get('/api/prompt-templates/:id', async (req, res) => {
    try {
      ...
      // Get related templates if forked
      const relatedTemplates = template.forkedFrom 
        ? await storage.getPromptTemplates({ forkedFrom: template.forkedFrom, limit: 5 })
        : [];
      
      res.json({ ...template, relatedTemplates });
    } catch (error) {
  ...
  app.put('/api/prompt-templates/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const updated = await storage.updatePromptTemplate(req.params.id, templateData);
      res.json(updated);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid template data", errors: error.errors });
  ...
  app.post('/api/prompt-templates/:id/fork', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const forked = await storage.forkTemplate(req.params.id, userId);
      res.status(201).json(forked);
    } catch (error) {
      console.error("Error forking prompt template:", error);
      res.status(500).json({ message: "Failed to fork prompt template" });
    }
  });

  app.post('/api/prompt-templates/:id/usage', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const { promptId, filledValues, output } = req.body;
      
      const usageData = insertTemplateUsageHistorySchema.parse({
        userId,
  ...
  app.get('/api/prompt-templates/most-used', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const { limit = "10" } = req.query;
      
      const templates = await storage.getMostUsedTemplates(userId, parseInt(limit as string));
      res.json(templates);
    } catch (error) {
      console.error("Error fetching most used templates:", error);
      res.status(500).json({ message: "Failed to fetch most used templates" });
    }
  });

  // Custom Synonyms API
  app.get('/api/custom-synonyms', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const { keywordId, limit = "50", offset = "0" } = req.query;
      ...
      const synonyms = await storage.getUserCustomSynonyms(options);
      res.json(synonyms);
    } catch (error) {
      console.error("Error fetching custom synonyms:", error);
      res.status(500).json({ message: "Failed to fetch custom synonyms" });
    }
  });

  app.post('/api/custom-synonyms', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const synonymData = insertUserCustomSynonymSchema.parse({
        ...req.body,
        userId,
      });
      
      const synonym = await storage.createUserCustomSynonym(synonymData);
      res.status(201).json(synonym);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid synonym data", errors: error.errors });
      }
      console.error("Error creating custom synonym:", error);
      res.status(500).json({ message: "Failed to create custom synonym" });
    }
  });

  app.put('/api/custom-synonyms/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      
      // Check ownership
      const synonym = await storage.getUserCustomSynonym(req.params.id);
      if (!synonym) {
        return res.status(404).json({ message: "Synonym not found" });
      }
      
      if (synonym.userId !== userId) {
        return res.status(403).json({ message: "Not authorized to update this synonym" });
      }
      
      const synonymData = insertUserCustomSynonymSchema.partial().parse(req.body);
      const updated = await storage.updateUserCustomSynonym(req.params.id, synonymData);
      res.json(updated);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid synonym data", errors: error.errors });
      }
      console.error("Error updating custom synonym:", error);
      res.status(500).json({ message: "Failed to update custom synonym" });
    }
  });

  app.delete('/api/custom-synonyms/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      ...
      if (synonym.userId !== userId) {
        return res.status(403).json({ message: "Not authorized to delete this synonym" });
      }
      
      await storage.deleteUserCustomSynonym(req.params.id);
      res.json({ message: "Synonym deleted successfully" });
    } catch (error) {
      console.error("Error deleting custom synonym:", error);
      res.status(500).json({ message: "Failed to delete custom synonym" });
    }
  });

  // Keyword Usage History API
  app.get('/api/keyword-usage-history', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const { keywordId, promptId, limit = "50", offset = "0" } = req.query;
      ...
      const history = await storage.getKeywordUsageHistory(options);
      res.json(history);
    } catch (error) {
      console.error("Error fetching keyword usage history:", error);
      res.status(500).json({ message: "Failed to fetch keyword usage history" });
    }
  });

  app.get('/api/keyword-usage-history/most-used', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const { limit = "10" } = req.query;
      
      const keywords = await storage.getMostUsedKeywords(userId, parseInt(limit as string));
      res.json(keywords);
    } catch (error) {
      console.error("Error fetching most used keywords:", error);
      res.status(500).json({ message: "Failed to fetch most used keywords" });
    }
  });

  // Template Usage History API
  app.get('/api/template-usage-history', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any).claims.sub;
      const { templateId, promptId, limit = "50", offset = "0" } = req.query;
      ...
      const history = await storage.getTemplateUsageHistory(options);
      res.json(history);
    } catch (error) {
      console.error("Error fetching template usage history:", error);
      res.status(500).json({ message: "Failed to fetch template usage history" });
    }
  });
